    ...
    <div className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen">
      ...
      <div className="p-6">
        ...
        <div className="space-y-6">
          {cartItems.length > 0 ? (
            cartItems.map((item) => (
              <div key={item.title} className="flex flex-col sm:flex-row items-center sm:justify-between bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition">
                <div className="flex flex-col sm:flex-row items-center">
                  {item.image && (
                    <Image src={urlFor(item.image).url()} className="w-32 h-32 object-cover rounded-md border-2 border-gray-300" alt="image" width={500} height={500} />
                  )}
                  <div className="sm:ml-4 mt-4 sm:mt-0">
                    <h2 className="text-lg font-bold text-gray-700 text-center sm:text-left">{item.title}</h2>
                    <p className="text-gray-500 text-center sm:text-left">Price: ${item.discountPrice}</p>
                    <div className="flex items-center mt-3 gap-2 justify-center sm:justify-start">
                      <button onClick={() => handleDecrement(item.title)} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">-</button>
                      <span className="font-bold text-gray-700">{item.quantity}</span>
                      <button onClick={() => handleIncrement(item.title)} className="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">+</button>
                    </div>
                  </div>
                </div>
                <button onClick={() => handleRemove(item.title)} className="mt-4 sm:mt-0 px-5 py-2 bg-red-600 text-white rounded hover:bg-red-700">Remove</button>
              </div>
            ))
          ) : (
            <div className="flex flex-col items-center justify-center h-96">
              <h2 className="text-2xl font-bold text-gray-700">Your cart is empty</h2>
              <Link href="/Shop" className="mt-4 bg-blue-500 text-white px-6 py-3 rounded shadow hover:bg-blue-600">Continue Shopping</Link>
            </div>
          )}
        </div>
        {cartItems.length > 0 && (
          <div className="mt-8 bg-white p-6 rounded-lg shadow-lg">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold">Total:</h2>
              <p className="text-xl font-extrabold text-gray-800">${calculateTotal().toFixed(2)}</p>
            </div>
            <button onClick={handleProceed} className="mt-4 w-full px-4 py-3 bg-gradient-to-r from-green-400 to-green-600 text-white rounded-lg shadow hover:from-green-500 hover:to-green-700">Proceed to Checkout</button>
          </div>
        )}
      </div>
      <Footer />
