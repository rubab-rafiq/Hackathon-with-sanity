...
// export interface TIProduct {
//   title: string;
//   image: any; // Ideally should be a specific type like `imageUrl`, `SanityImageType`, etc.
//   description: string;
//   price: number;
...
// const ProductDetails = ({ product }: { product: TIProduct }) => {
//   const [selectedColor, setSelectedColor] = useState<string | null>(null);
//   const [selectedSize, setSelectedSize] = useState<string | null>(null);
//   const [isLoading, setIsLoading] = useState<boolean>(false);
...
// Define Product Details interface with slug and quantity
export interface TIProduct {
  title: string;
  image: any; // Ideally should be a specific type like `imageUrl`, `SanityImageType`, etc.
  description: string;
...
const ProductDetails = ({ product }: { product: TIProduct }) => {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      // Add slug and quantity to the cart data
      await addToCart({ ...product, colors: [selectedColor], sizes: [selectedSize], quantity: 1 });
      toast.success("Product added to cart successfully!");
    } catch (error) {
      console.error("Error adding to cart:", error);
      toast.error("Failed to add product to cart!");
